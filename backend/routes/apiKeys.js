...
router.post('/', async (req, res) => {
  ...
  const { openai_key, elevenlabs_key, creatomate_key, youtube_credentials } = req.body;
      ...
      if (row) {
        ...
        db.run(
          `UPDATE api_keys SET 
            ...
            updated_at = ?
          WHERE user_id = ?`,
          [openai_key, elevenlabs_key, creatomate_key, youtube_credentials, now, userId],
          (err) => {
            if (err) {
        ...
        // Insert new keys
        db.run(
          `INSERT INTO api_keys (user_id, openai_key, elevenlabs_key, creatomate_key, youtube_credentials, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [userId, openai_key, elevenlabs_key, creatomate_key, youtube_credentials, now, now],
          (err) => {
            if (err) {
