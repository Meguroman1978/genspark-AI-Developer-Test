...
// Download script as RTF or TXT
router.get('/download-script/:jobId', (req, res) => {
  const db = req.app.locals.db;
  const { jobId } = req.params;
  const format = req.query.format || 'txt'; // 'txt' or 'rtf'

  db.get(
    'SELECT theme, script_text, created_at FROM video_jobs WHERE id = ?',
    [jobId],
    (err, job) => {
      if (err) {
      ...
      if (!job || !job.script_text) {
        return res.status(404).json({ error: 'Script not found' });
      }

      const filename = `script_${job.theme.replace(/[^a-zA-Z0-9]/g, '_')}_${jobId}`;
      
      if (format === 'rtf') {
        // Generate RTF format (compatible with Word, TextEdit, WordPad)
        const rtfContent = generateRTF(job.theme, job.script_text, job.created_at);
        res.setHeader('Content-Type', 'application/rtf');
        res.setHeader('Content-Disposition', `attachment; filename="${filename}.rtf"`);
        res.send(rtfContent);
      } else {
        // Generate plain text format (UTF-8, compatible with all text editors)
        const txtContent = generateTXT(job.theme, job.script_text, job.created_at);
...
// Helper function to generate RTF format
function generateRTF(theme, script, createdAt) {
  // RTF format with UTF-8 support (compatible with Windows/Mac Word, TextEdit, WordPad)
...
\\pard\\f1\\fs28\\b Video Script\\b0\\par
\\par
\\f1\\fs24\\b Theme: \\b0 ${escapeRTF(theme)}\\par
\\b Generated: \\b0 ${date}\\par
\\par
\\line
\\par
\\f0\\fs22 ${escapeRTF(script)}\\par
}`;
...
function generateTXT(theme, script, createdAt) {
  const date = new Date(createdAt).toLocaleString('ja-JP');
  
  return `========================================
動画スクリプト / Video Script
========================================
...
----------------------------------------
Generated by AI Video Generator
========================================`;
}
