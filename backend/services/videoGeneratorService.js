  ...
  async generateVideo(config) {
    ...
    try {
      ...
      if (keys.youtubeCredentials) {
        ...
        const finalTitle = videoTitle || this.generateDefaultTitle(theme);
        // Use custom description or generate default with Japanese/English bilingual template
        const finalDescription = videoDescription || this.generateDefaultDescription(theme, duration);
...
Return your response in the following JSON format:
{
  ...
  generateDefaultTitle(theme) {
    ...
    return `日本の諺：${theme} | Japanese Proverb: ${theme}`;
  }
